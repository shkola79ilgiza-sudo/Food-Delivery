...
const ChefProcurementPlanner = ({ onClose }) => {
  ...
  const [removedRecommendations, setRemovedRecommendations] = useState(new Set()); // Отслеживаем удаленные рекомендации
  ...
  const getIngredientIcon = (ingredient) => {
    ...
    // Мясо и птица
    if (normalizedIngredient.includes('курица') || normalizedIngredient.includes('куриное')) return '🍗';
    if (normalizedIngredient.includes('говядина') || normalizedIngredient.includes('говяжье')) return '🥩';
    if (normalizedIngredient.includes('свинина') || normalizedIngredient.includes('свиное')) return '🐷';
    if (normalizedIngredient.includes('баранина') || normalizedIngredient.includes('баранье')) return '🐑';
    if (normalizedIngredient.includes('рыба') || normalizedIngredient.includes('лосось') || normalizedIngredient.includes('треска')) return '🐟';
    
    // Овощи
    if (normalizedIngredient.includes('картофель') || normalizedIngredient.includes('картошка')) return '🥔';
    if (normalizedIngredient.includes('лук')) return '🧅';
    if (normalizedIngredient.includes('морковь') || normalizedIngredient.includes('морковка')) return '🥕';
    if (normalizedIngredient.includes('помидор') || normalizedIngredient.includes('томат')) return '🍅';
    ...
    if (normalizedIngredient.includes('сыр') || normalizedIngredient.includes('пармезан')) return '🧀';
    ...
    // Орехи
    if (normalizedIngredient.includes('орех') || normalizedIngredient.includes('миндаль')) return '🥜';
    ...
    // Консервы
    if (normalizedIngredient.includes('тушенка') || normalizedIngredient.includes('консерв')) return '🥫';
...
соль, перец, зелень по вкусу`,
  ...
  const getRealDishIngredients = (dishName, chefDishes) => {
    ...
    console.log('📋 Доступные блюда повара:', chefDishes.map(d => d.name));
  ...
  const parseIngredientsText = (ingredientsText) => {
    ...
    lines.forEach(line => {
      ...
      if (trimmedLine.includes('по вкусу') || trimmedLine.includes('специи') || trimmedLine.includes('приправы')) {
        return; // Пропускаем эту строку
      }
      
      // Парсим строки вида "500г говядина" или "2 шт лук"
      const match = trimmedLine.match(/^(\d+(?:\.\d+)?)\s*(г|кг|шт|л|мл|ст\.л|ч\.л|пучок|зубчик|долька|кусок|порция)?\s*(.+)$/i);
      ...
      if (match) {
        ...
        else if (unit === 'л' || unit === 'мл') amountInGrams = amount; // Для жидкостей оставляем как есть
        else if (unit === 'шт' || unit === 'пучок' || unit === 'зубчик' || unit === 'долька' || unit === 'кусок' || unit === 'порция') {
  ...
  const getDishIngredients = (dishName) => {
    ...
    return ingredientsMap[dishName] || [
  ...
  const getIngredientPrice = (ingredient) => {
    ...
    const prices = {
      // Мясо и птица
      'Куриная грудка': 300, 'курица': 300, 'куриное мясо': 300,
      'Говядина': 500, 'говяжье мясо': 500, 'говяжий фарш': 450,
      'Свинина': 400, 'свиное мясо': 400, 'свиной фарш': 350,
      'Баранина': 600, 'баранье мясо': 600,
      'Рыба': 300, 'лосось': 800, 'треска': 400, 'минтай': 200,
      
      // Овощи
      'Картофель': 50, 'картошка': 50,
      'Лук': 80, 'лук репчатый': 80, 'лук зеленый': 120,
      'Морковь': 60, 'морковка': 60,
      'Помидоры': 150, 'помидор': 150, 'томаты': 150,
      'Огурцы': 120, 'огурец': 120,
      'Капуста': 40, 'капуста белокочанная': 40,
      'Перец': 200, 'перец болгарский': 200,
      'Чеснок': 300, 'чеснок': 300,
      
      // Зелень
      'Салат': 150, 'салат айсберг': 150, 'салат листовой': 150,
      'Петрушка': 100, 'петрушка': 100,
      'Укроп': 100, 'укроп': 100,
      'Базилик': 200, 'базилик': 200,
      
      // Молочные продукты
      'Молоко': 80, 'молоко': 80,
      'Сыр': 400, 'сыр': 400, 'пармезан': 800, 'чеддер': 500,
      'Творог': 200, 'творог': 200,
      'Сметана': 150, 'сметана': 150,
      'Масло сливочное': 200, 'масло': 200,
      
      // Крупы и мука
      'Мука': 40, 'мука': 40, 'мука пшеничная': 40,
      'Рис': 80, 'рис': 80, 'рис длиннозерный': 100,
      'Гречка': 100, 'гречка': 100,
      'Овсянка': 60, 'овсянка': 60,
      
      // Орехи и сухофрукты
      'Грецкие орехи': 600, 'орехи грецкие': 600,
      'Миндаль': 800, 'миндаль': 800,
      'Изюм': 300, 'изюм': 300,
      
      // Специи и приправы
      'Соль': 20, 'соль': 20,
      'Перец черный': 100, 'перец': 100,
      'Лавровый лист': 50, 'лаврушка': 50,
      
      // Масла
      'Масло растительное': 100, 'масло подсолнечное': 100,
      'Масло оливковое': 300, 'оливковое масло': 300,
      
      // Яйца
      'Яйца': 80, 'яйцо': 80, 'яйца куриные': 80,
      
      // Сахар и сладости
      'Сахар': 60, 'сахар': 60, 'сахар песок': 60,
      'Мед': 300, 'мед': 300,
      
      // Консервы
      'Тушенка': 200, 'тушенка говяжья': 200,
      'Консервы': 150, 'консервы рыбные': 150
    };
    ...
    for (const [key, price] of Object.entries(prices)) {
      if (normalizedIngredient.includes(key.toLowerCase()) || key.toLowerCase().includes(normalizedIngredient)) {
        return price;
      }
    }
  ...
  const generateRecommendations = (orders) => {
    ...
    orders.forEach(order => {
      ...
      order.items?.forEach(item => {
        console.log('📦 Анализируем товар:', item.name, 'количество:', item.quantity);
        
        // Суммируем точное количество порций
        dishPortions[item.name] = (dishPortions[item.name] || 0) + item.quantity;
      });
    });
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const chefOrders = clientOrders.filter(order => {
        ...
        return order.items?.some(item => item.chefId === chefId) && 
  ...
  const handleAddToShoppingList = (recommendation) => {
    try {
      ...
      if (recommendation.type === 'dish') {
        const chefId = localStorage.getItem('chefId') || 'demo-chef-1';
        const chefDishes = getChefDishes(chefId);
        const ingredients = getRealDishIngredients(recommendation.name, chefDishes);
        
        console.log('🍽️ Добавляем блюдо в список покупок:', recommendation.name);
        ...
        ingredients.forEach(ingredient => {
          ...
          if (existingItem) {
            ...
            existingItem.suggestedAmount += Math.ceil(ingredient.amount * recommendation.suggestedAmount);
            existingItem.estimatedCost = Math.ceil(existingItem.suggestedAmount * getIngredientPrice(ingredient.name));
          } else {
            ...
            shoppingList.push({
              ...
              suggestedAmount: Math.ceil(ingredient.amount * recommendation.suggestedAmount),
              unit: ingredient.unit || 'кг',
              estimatedCost: Math.ceil(ingredient.amount * recommendation.suggestedAmount * getIngredientPrice(ingredient.name)),
      ...
      const existingItem = shoppingList.find(item => item.name === recommendation.name);
      ...
      setRemovedRecommendations(prev => new Set([...prev, recommendation.id]));
      
      showSuccess(`${recommendation.name} добавлен в список покупок и удален из рекомендаций`);
    } catch (error) {
  ...
  return (
    <div className="chef-procurement-modal-overlay" onClick={onClose}>
      <div className="chef-procurement-modal" onClick={(e) => e.stopPropagation()}>
          ...
          {removedRecommendations.size > 0 && (
            <div className="recommendations-section">
              ...
              <div className="recommendations-list">
                {recommendations
                  .filter(recommendation => removedRecommendations.has(recommendation.id))
                  .map(recommendation => (
                    <div key={recommendation.id} className="recommendation-card" style={{ opacity: 0.7, border: '2px dashed #ccc' }}>
                      ...
                      <div className="recommendation-details">
                        <div className="detail-item">
                          <strong>Заказано порций:</strong> {recommendation.actualPortions || 0}
                        </div>
                        <div className="detail-item">
                          <strong>Рекомендуется:</strong> {recommendation.suggestedAmount} {recommendation.unit} (+20% запас)
                        </div>
                        <div className="detail-item">
                          <strong>Цена за порцию:</strong> {recommendation.dishPrice || 200}₽
                        </div>
                        <div className="detail-item">
                          <strong>Общая стоимость:</strong> {recommendation.estimatedCost}₽
                        </div>
                      </div>
                      ...
                      <div className="recommendation-actions">
                        <button 
                          ...
                          onClick={() => handleRestoreRecommendation(recommendation.id)}
            ...
            <div className="recommendations-list">
              {recommendations
                ...
                <div key={recommendation.id} className="recommendation-card">
                  ...
                  <div className="recommendation-actions">
                    <button 
                      className={`add-to-list-btn ${addedToShoppingList.has(recommendation.id) ? 'added' : ''}`}
                      onClick={() => handleAddToShoppingList(recommendation)}
                      disabled={addedToShoppingList.has(recommendation.id)}
                    >
                      {addedToShoppingList.has(recommendation.id) ? '✅ Добавлено' : '➕ Добавить в список покупок'}
                    </button>
                  </div>
                </div>
              ))}
