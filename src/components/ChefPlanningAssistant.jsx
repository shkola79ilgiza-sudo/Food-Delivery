...
const ChefPlanningAssistant = ({ chefId }) => {
  ...
  const generateDemandForecast = (holidayDemand, dishStats) => {
    ...
    if (currentMonth === 12 || currentMonth === 1) {
      ...
      daysToHoliday = currentMonth === 12 ? 31 - currentDay : 31 - currentDay + 31;
    ...
    if (nextHoliday && holidayDemand[nextHoliday]) {
      ...
      forecast[nextHoliday] = {
        ...
        daysToHoliday,
        recommendations: generateHolidayRecommendations(nextHoliday, holidayData)
      };
    }
  ...
  const generateHolidayRecommendations = (holiday, holidayData) => {
    ...
    if (holiday === 'новый год') {
      recommendations.push({
        ...
        description: 'Добавьте праздничные блюда: оливье, селедка под шубой, торт',
        priority: 'high'
      });
      recommendations.push({
        type: 'inventory',
        title: 'Закупки',
        description: 'Закупите заранее: майонез, селедку, мандарины, шампанское',
        priority: 'high'
      });
  ...
  const generatePlanningRecommendations = (dishStats, categoryStats, seasonalStats) => {
    const recommendations = [];
    ...
    if (topDishes.length > 0) {
      recommendations.push({
        ...
        description: 'Эти блюда заказывают чаще всего - убедитесь, что они всегда в наличии',
    ...
    if (topSeasonalDishes.length > 0) {
      recommendations.push({
        ...
        description: `В ${getSeasonName(currentSeason).toLowerCase()} популярны эти блюда`,
        dishes: topSeasonalDishes.map(([dish, count]) => ({ name: dish, count })),
        priority: 'medium'
      });
    }
    ...
    if (lowStockDishes.length > 0) {
      recommendations.push({
        ...
        description: 'Эти блюда заказывают, но они приносят мало дохода. Рассмотрите оптимизацию',
  ...
  const performAnalysis = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.log('👨‍🍳 ChefPlanningAssistant: Holiday demand analyzed:', holidayDemand);

        // Анализируем популярность блюд
        const { dishStats, categoryStats, seasonalStats } = analyzeDishPopularity(allOrders);
        console.log('👨‍🍳 ChefPlanningAssistant: Dish popularity analyzed:', dishStats);

        // Генерируем прогноз спроса
        const forecast = generateDemandForecast(holidayDemand, dishStats);
        console.log('👨‍🍳 ChefPlanningAssistant: Demand forecast generated:', forecast);

        // Генерируем рекомендации по планированию
        const recommendations = generatePlanningRecommendations(dishStats, categoryStats, seasonalStats);
        console.log('👨‍🍳 ChefPlanningAssistant: Planning recommendations generated:', recommendations);

        setDemandForecast(forecast);
        setPlanningRecommendations(recommendations);
        setIsAnalyzing(false);

        showSuccess(`Анализ завершен! Сгенерировано ${recommendations.length} рекомендаций`);
      } catch (error) {
  ...
  return (
    <div className="chef-planning-assistant" style={{
      ...
      <div className="assistant-header" style={{
        ...
        <h3 style={{ margin: 0, color: '#333' }}>👨‍🍳 Помощник по планированию</h3>
        <button
          onClick={performAnalysis}
          disabled={isAnalyzing}
          style={{
            background: isAnalyzing ? '#ccc' : 'linear-gradient(135deg, #ff6b6b, #ff8e53)',
            color: 'white',
            border: 'none',
            padding: '10px 20px',
            borderRadius: '8px',
      ...
      {allOrders.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: '18px', marginBottom: '10px' }}>Нет данных для анализа</div>
          <div style={{ fontSize: '14px' }}>Дождитесь первых заказов для получения рекомендаций</div>
        </div>
      ) : (
        <div className="assistant-content">
          {/* Прогноз спроса */}
          {demandForecast && Object.keys(demandForecast).length > 0 && (
            <div className="demand-forecast" style={{
              ...
              <h4 style={{ margin: '0 0 15px 0', fontSize: '18px' }}>📈 Прогноз спроса</h4>
              {Object.entries(demandForecast).map(([holiday, forecast]) => (
                <div key={holiday} style={{
                  ...
                  <div style={{ fontWeight: 'bold', marginBottom: '10px', fontSize: '16px' }}>
                    {holiday === 'новый год' ? 'Новый год' :
                     holiday === '8 марта' ? '8 марта' :
                     holiday === 'навруз' ? 'Навруз' :
                     holiday === 'ураза' ? 'Ураза-байрам' :
                     holiday === 'масленица' ? 'Масленица' : holiday}
                  </div>
                  <div style={{ fontSize: '14px', marginBottom: '8px' }}>
                    Ожидается рост заказов на <strong>{forecast.expectedGrowth}%</strong>
                  </div>
                  <div style={{ fontSize: '14px', marginBottom: '10px' }}>
                    Популярные блюда: {forecast.popularDishes.map(d => d.dish).join(', ')}
                  </div>
          ...
          {planningRecommendations.length > 0 && (
            <div className="planning-recommendations">
              <h4 style={{ color: '#333', marginBottom: '15px' }}>💡 Рекомендации по планированию:</h4>
              <div style={{
                display: 'grid',
                      ...
                      <div style={{ fontSize: '12px' }}>
                        ...
                        <ul style={{ margin: '5px 0 0 15px', padding: 0 }}>
                          {rec.dishes.slice(0, 3).map((dish, i) => (
                            <li key={i}>
                              {dish.name} {dish.orders ? `(${dish.orders} заказов)` : ''}
