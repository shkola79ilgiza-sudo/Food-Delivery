...
const IntegratedPricingDemo = () => {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const integrated = await integratedPricingSystem.getIntegratedPrice(productName, region);
      ...
      setSourceData({
        ...
        crowdsourcing: crowdsourcing.status === 'fulfilled' ? crowdsourcing.value : null,
        mockData: mockData.status === 'fulfilled' ? mockData.value : null
      });

      // Получаем рекомендации
      const recs = integratedPricingSystem.getPricingRecommendations(productName, region);
        ...
        <div style={{ display: 'grid', gap: '15px' }}>
          ...
          {sourceData.openData && (
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#6c757d' }}>
                Диапазон: {sourceData.openData.minPrice}₽ - {sourceData.openData.maxPrice}₽ | 
                Уверенность: {Math.round(sourceData.openData.confidence * 100)}%
              </div>
            </div>
          )}
          ...
          {sourceData.crowdsourcing && (
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#6c757d' }}>
                ...
                Уверенность: {Math.round(sourceData.crowdsourcing.confidence * 100)}% | 
                {sourceData.crowdsourcing.verified ? '✅ Верифицировано' : '⏳ Ожидает подтверждения'}
              </div>
            </div>
          )}
          ...
          {sourceData.mockData && (
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#6c757d' }}>
                Базовая цена: {sourceData.mockData.basePrice}₽ | 
                Региональный множитель: {sourceData.mockData.regionalMultiplier}x | 
                Сезонный фактор: {sourceData.mockData.seasonalFactor}x | 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {recommendations.map((rec, index) => (
              <div key={index} style={{
                ...
                borderRadius: '6px',
                border: `1px solid ${rec.type === 'warning' ? '#ffeaa7' : '#bee5eb'}`,
                fontSize: '14px'
              }}>
      ...
      {statistics && (
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
            <div>
              <strong>Открытые API:</strong><br />
              Запросов: {statistics.openData.totalRequests}<br />
              Кэш: {Math.round(statistics.openData.cacheHitRate * 100)}%
            </div>
            <div>
              <strong>Краудсорсинг:</strong><br />
              Продуктов: {statistics.crowdsourcing.totalProducts}<br />
              Верифицировано: {statistics.crowdsourcing.verifiedProducts}
            </div>
            <div>
              <strong>Веса источников:</strong><br />
              Открытые API: {Math.round(statistics.weights.openData * 100)}%<br />
              Краудсорсинг: {Math.round(statistics.weights.crowdsourcing * 100)}%<br />
