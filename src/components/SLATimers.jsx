...
const SLATimers = ({ order, onSLAViolation, onCompensation }) => {
  ...
  const checkSLA = () => {
    ...
    try {
      ...
      if (order.status === 'pending_confirmation' && timeDiff > slaThresholds.acceptance) {
      ...
      if (order.status === 'preparing') {
        const preparationStartTime = order.preparationStartTime || order.cookingStartTime;
        const preparationTime = preparationStartTime ? 
          (now - new Date(preparationStartTime)) / (1000 * 60) : timeDiff;
        
        const estimatedTime = order.estimatedPreparationTime || order.cookingDuration || 30;
    ...
    if (order.status === 'delivering') {
      const deliveryTime = order.deliveryStartTime ? 
        (now - new Date(order.deliveryStartTime)) / (1000 * 60) : 0;
      
      if (deliveryTime > (order.estimatedDeliveryTime || 30) + slaThresholds.delivery) {
        if (!violations.find(v => v.type === 'delivery')) {
          newViolations.push({
            id: `delivery-${Date.now()}`,
            type: 'delivery',
            message: 'Превышено время доставки',
            threshold: (order.estimatedDeliveryTime || 30) + slaThresholds.delivery,
  ...
  const updateSLAStatus = (timeDiff) => {
  ...
  const applyCompensation = (violation) => {
    ...
    showSuccess(`💰 Компенсация ${violation.compensation}₽ применена автоматически`);
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ 
            ...
            {slaStatus === 'on_track' ? 'В норме' :
             slaStatus === 'warning' ? 'Предупреждение' :
             slaStatus === 'violated' ? 'Нарушение' :
             slaStatus === 'completed' ? 'Завершено' : 'Ожидание'}
          </span>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        <div style={{
          ...
          <span style={{ 
            ...
            {timeRemaining > 0 ? `${Math.round(timeRemaining)} мин осталось` : 'Время истекло'}
          </span>
        </div>
        <div style={{
          width: '100%',
          ...
          <div style={{
            width: `${progress}%`,
            height: '100%',
            backgroundColor: progress > 80 ? '#F44336' : progress > 60 ? '#FF9800' : '#4CAF50',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {violations.map(violation => (
              <div
                ...
                style={{
                  padding: '12px',
                  background: violation.severity === 'critical' ? '#ffebee' : '#fff3e0',
                  border: `2px solid ${violation.severity === 'critical' ? '#F44336' : '#FF9800'}`,
                  borderRadius: '8px',
                  display: 'flex',
                  justifyContent: 'space-between',
                  alignItems: 'center'
              ...
              >
                <div>
                  <div style={{ 
                    fontSize: '14px', 
                    fontWeight: 'bold', 
                    color: violation.severity === 'critical' ? '#F44336' : '#FF9800',
                    marginBottom: '4px'
                  }}>
                    {violation.message}
                  </div>
                  <div style={{ fontSize: '12px', color: '#666' }}>
                    Порог: {violation.threshold} мин | Факт: {violation.actual} мин
                  </div>
                </div>
                <button
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>📊 Статистика заказа:</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '10px' }}>
          <div>Статус: <strong>{order.status}</strong></div>
          <div>Время выполнения: <strong>{Math.round((new Date() - new Date(order.createdAt)) / (1000 * 60))} мин</strong></div>
          <div>Нарушения: <strong>{violations.length}</strong></div>
          <div>Компенсации: <strong>{compensations.reduce((sum, c) => sum + c.amount, 0)}₽</strong></div>
        </div>
      </div>
    </div>
  );
