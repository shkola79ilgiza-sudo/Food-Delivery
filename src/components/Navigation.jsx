...
const ChefLink = () => {
  ...
  return chefId ? (
    <Link to={`/chef/${encodeURIComponent(chefId)}/menu`}>{t.chefPage}</Link>
  ) : null;
};
...
const ClientLink = () => {
  ...
  const label =
    typeof t.clientMenu === "string"
      ? t.clientMenu
      : t.clientMenu?.title || "Меню";
  return token && role === "client" ? (
    <Link to="/client/menu">{label}</Link>
  ) : null;
};
...
const AICoachLink = () => {
  ...
  const role = localStorage.getItem("role");
  return token && role === "client" ? (
    <Link to="/client/ai-coach">🍎 Помощник по питанию</Link>
  ) : null;
};
...
const AIChefAssistantLink = () => {
  const token = localStorage.getItem("authToken");
  const role = localStorage.getItem("role");
  return token && role === "chef" ? (
    <Link to="/chef/ai-assistant">🤖 AI-Помощник</Link>
  ) : null;
};
      ...
      <div className="nav-track">
        ...
        <div className="nav-section">
          <span className="nav-label">
            {typeof t.chefs === "string" ? t.chefs : "Повара"}:
          </span>
        ...
        <div className="nav-section">
          <span className="nav-label">
            {typeof t.clients === "string" ? t.clients : "Клиенты"}:
          </span>
        ...
        <div className="nav-section">
          <span className="nav-label">
            {typeof t.admin === "string" ? t.admin : "Админ"}:
          </span>
          <Link to="/admin/login">
            {typeof t.adminPanel === "string" ? t.adminPanel : "Админ-панель"}
          </Link>
        </div>
