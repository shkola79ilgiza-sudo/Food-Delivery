...
const Gamification = ({ userOrderHistory = [], userProfile = {} }) => {
    ...
    () => [
      ...
      },
      {
        ...
        condition: (history) => {
          ...
          return (
            last7Days.length >= 7 &&
            !last7Days.some((order) => order.category === "fast_food")
          );
        },
        unlocked: false,
      },
      ...
      },
      {
        ...
        condition: (history) => {
          ...
          return (
            last3Days.length >= 3 &&
            last3Days.every(
              (order) => order.calories >= 1200 && order.calories <= 2000
            )
          );
        },
        unlocked: false,
      },
      {
        ...
        xpReward: 60,
        condition: (history) => {
          const breakfastOrders = history.filter(
            (order) =>
      ...
      },
      {
        ...
        condition: (history) => {
          ...
          const monthlyOrders = history.filter(
            (order) => new Date(order.date || order.createdAt) >= monthAgo
          );
  ...
  useEffect(() => {
    ...
    checkAchievements();
  }, [userOrderHistory, achievements, achievementConfig]);

  // Расчет текущего уровня
  useEffect(() => {
    const currentLevel = Object.keys(levelConfig)
      .reverse()
      .find((level) => experience >= levelConfig[level].xpRequired);
    setLevel(parseInt(currentLevel) || 1);
  }, [experience, levelConfig]);
        ...
        >
          ...
          <div style={{ marginBottom: "15px" }}>
            ...
            >
              ...
              <span>
                {nextLevelProgress.xpNeeded > 0
                  ? `До следующего уровня: ${nextLevelProgress.xpNeeded} XP`
              ...
              <div
                style={{
                  ...
                  background: `linear-gradient(90deg, ${
                    ...
                    levelConfig[level + 1]?.color || levelConfig[level].color
                  })`,
