...
const ClientCookingRequests = () => {
  ...
  const loadCookingRequests = useCallback(() => {
    try {
      const requests = JSON.parse(
        localStorage.getItem("cookingRequests") || "[]"
      );
  ...
  useEffect(() => {
    if (cookingRequests.length > 0) {
      const highlightRequestId = localStorage.getItem(
        "highlightCookingRequestId"
      );
      if (highlightRequestId) {
        localStorage.removeItem("highlightCookingRequestId");
        const requestElement = document.getElementById(
          `cooking-request-${highlightRequestId}`
        );
        if (requestElement) {
  ...
  return (
    <div
      className="client-cooking-requests"
      style={{
    ...
    >
      <div
        className="client-cooking-requests-header"
        style={{
          display: "flex",
      ...
      {cookingRequests.length === 0 ? (
        ...
        >
          ...
          <h3 style={{ color: "#2c3e50", marginBottom: "10px" }}>
            Нет запросов на приготовление
          </h3>
          <p style={{ color: "#6c757d", fontSize: "16px" }}>
            Вы еще не отправляли запросы на приготовление блюд из своих
            продуктов
          </p>
        </div>
      ) : (
