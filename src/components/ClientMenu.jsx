...
const ClientMenu = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "clientNotificationsUpdated",
      handleNotificationsUpdated
    );

    return () => {
      window.removeEventListener(
        "clientNotificationAdded",
    ...
    const loadCart = () => {
      ...
      if (savedCart) {
        try {
          ...
          console.log(
            "  - Size in KB:",
            (new Blob([savedCart]).size / 1024).toFixed(2)
          );
  ...
  const isDishMatchingFilters = (dish) => {
    ...
    if (dish.calories) {
      if (
        dish.calories < calorieFilter.min ||
        dish.calories > calorieFilter.max
      ) {
        return false;
      }
    }
  ...
  const handleCookingRequestSubmit = async (requestData) => {
    try {
      ...
      showSuccess(
        "Запрос на гото☕ку отпра☕лен! По☕ар с☕яжется с ☕ами ☕ ближайшее ☕ремя."
      );
    } catch (error) {
  ...
  const handleHelpGuestRequestSubmit = async (requestData) => {
    try {
      ...
      const existingRequests = JSON.parse(
        localStorage.getItem("helpGuestRequests") || "[]"
      );
      ...
      const updatedChefNotifications = [
        chefNotification,
        ...chefNotifications,
      ].slice(0, 50);
      localStorage.setItem(
        "chefNotifications",
        JSON.stringify(updatedChefNotifications)
      );
      ...
      showSuccess(
        "Запрос на помощь ☕ подгото☕ке к приему гостей отпра☕лен! По☕ар с☕яжется с ☕ами ☕ ближайшее ☕ремя."
      );
    } catch (error) {
  ...
  const handleMasterClassRequestSubmit = async (requestData) => {
    try {
      ...
      // В реальном приложении это будет API запрос
      showSuccess(
        "Зая☕ка на мастер-класс отпра☕лена! По☕ар с☕яжется с ☕ами для уточнения деталей."
      );
    } catch (error) {
    ...
    try {
      ...
      if (error.name === "QuotaExceededError") {
        ...
        try {
          ...
          setCart(minimalCart);
          showSuccess(
            `"${dish.name}" ${t.dishAddedToCart} (корзина была очищена из-за пре☕ышения лимита)`
          );
        } catch (retryError) {
          console.error("Failed to save even minimal cart:", retryError);
          showError(
            "Ошибка сохранения корзины. Попробуйте очистить данные браузера."
          );
        }
      } else {
  ...
  const loadReviews = () => {
    const savedReviews = JSON.parse(
      localStorage.getItem("dishReviews") || "[]"
    );
  ...
  const getDishRating = (dishId) => {
    ...
    return (
      dishReviews.reduce((sum, review) => sum + review.rating, 0) /
      dishReviews.length
    );
  };
  ...
  return (
    <div
      className="client-menu-container"
      style={{
        backgroundImage:
      ...
      >
        ...
        <div className="client-menu-actions">
          ...
          <button
            ...
            className={`modern-button light nav-button ${
              showClientNotifications ? "active" : ""
            }`}
            style={{
              display: "flex",
              alignItems: "center",
          ...
          <button
            onClick={() => {
              console.log(
                'Кнопка "Мои запросы на помощь" нажата. Текущее состояние:',
                showHelpGuestRequests
              );
              setShowHelpGuestRequests(!showHelpGuestRequests);
            }}
            className={`modern-button light nav-button ${
              showHelpGuestRequests ? "active" : ""
            }`}
            style={{ display: "flex", alignItems: "center", gap: "6px" }}
          >
        ...
        >
          <div className="categories-section">
            ...
            {showHolidayAnalytics && (
              ...
              >
                <HolidayAnalytics
                  onClose={() => setShowHolidayAnalytics(false)}
                />
              </div>
            )}
            ...
            >
              ...
              >
                Найдено блюд: <strong>{filteredDishes.length}</strong> из{" "}
                {dishes.length}
              </div>
            </div>
              ...
              {sortedDishes.map((dish) => (
                ...
                >
                    ...
                    <div className="dish-actions">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          navigate(
                            `/dish/${
                              dish.chef?.id || dish.chefId || "unknown"
                            }/${dish.id}/passport`
                          );
                        }}
                        title="Паспорт блюда"
                  ...
                  <div className="dish-info">
                    ...
                    {showDiabeticMenu && (
                      <div
                        className="diabetic-indicators"
                        style={{ marginBottom: "10px" }}
                      >
                        {dish.carbs !== undefined && (
                          <span
                            className="diabetic-badge"
                            style={{
                              display: "inline-block",
                              background:
                                dish.carbs <= 20
          ...
          >
            <AIDishChatbot
              ...
              onEscalateToChef={(data) => {
                ...
                navigate(
                  `/client/chef-chat/${selectedChatbotDish.chefId || "unknown"}`
                );
              }}
              onClose={() => {
