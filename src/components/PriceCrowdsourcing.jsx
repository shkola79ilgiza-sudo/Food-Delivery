...
const PriceCrowdsourcing = ({ productName, onPriceAdded }) => {
  ...
  const handleVerify = (productName, location, userId) => {
    const success = crowdsourcingPrices.verifyPrice(productName, location, userId);
    if (success) {
            ...
            {existingPrices.map((priceData, index) => (
              <div key={index} style={{
                ...
                <div style={{ 
                  ...
                  Подтверждений: {priceData.verificationCount} | 
                  Источников: {priceData.sources} | 
                  {priceData.verified ? '✅ Верифицировано' : '⏳ Ожидает подтверждения'}
                </div>
                
                <button
                  onClick={() => handleVerify(productName, priceData.location, localStorage.getItem('userId') || 'anonymous')}
                  style={{
                    padding: '6px 12px',
                    background: '#27ae60',
                    color: 'white',
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <div>Средняя уверенность: {Math.round(stats.averageConfidence * 100)}%</div>
              </>
            );
          })()}
        </div>
