...
const ChefTiers = ({ chefId, onTierChange }) => {
  ...
  const loadChefTier = () => {
    try {
      ...
      const savedTierData = JSON.parse(localStorage.getItem(`chef_tier_data_${chefId}`) || '{}');
  ...
  const upgradeTier = (newTier) => {
    try {
      ...
      localStorage.setItem(`chef_tier_data_${chefId}`, JSON.stringify({
        tier: newTier,
  ...
  const getTierProgress = () => {
    ...
    const chefOrders = JSON.parse(localStorage.getItem('clientOrders') || '[]')
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ 
              ...
              {getCurrentTierInfo().price === 0 ? 'Бесплатно' : `${getCurrentTierInfo().price}₽/месяц`}
            </p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '15px' }}>
          <div style={{
            ...
            <span style={{ fontSize: '14px', fontWeight: 'bold' }}>Блюда в меню</span>
            <span style={{ fontSize: '14px', color: '#666' }}>
              {progress.dishesUsed} / {progress.dishesLimit === -1 ? '∞' : progress.dishesLimit}
            </span>
          </div>
          {progress.dishesLimit !== -1 && (
            <div style={{
              ...
              <div style={{
                width: `${Math.min((progress.dishesUsed / progress.dishesLimit) * 100, 100)}%`,
                height: '100%',
        ...
        <div style={{
          ...
          <span style={{ fontSize: '14px', fontWeight: 'bold' }}>Заказы сегодня</span>
          <span style={{ fontSize: '14px', color: '#666' }}>
            {progress.ordersToday} / {progress.ordersLimit === -1 ? '∞' : progress.ordersLimit}
          </span>
        </div>
        {progress.ordersLimit !== -1 && (
          <div style={{
            ...
            <div style={{
              width: `${Math.min((progress.ordersToday / progress.ordersLimit) * 100, 100)}%`,
              height: '100%',
              backgroundColor: getCurrentTierInfo().color,
              transition: 'width 0.3s ease'
            }} />
        ...
        <div style={{
          ...
          <span style={{ fontSize: '14px', fontWeight: 'bold' }}>Комиссия платформы</span>
          <span style={{ 
            fontSize: '16px', 
            fontWeight: 'bold', 
            color: getCurrentTierInfo().color 
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ color: '#666', fontSize: '12px' }}>Заказов за месяц</div>
          ...
          <div>
            <div style={{ color: '#666', fontSize: '12px' }}>Комиссия платформы</div>
            <div style={{ fontWeight: 'bold', color: '#2D5016' }}>
              {Math.round(JSON.parse(localStorage.getItem('clientOrders') || '[]')
                .filter(order => order.chefId === chefId)
                .reduce((sum, order) => sum + (order.total || 0), 0) * progress.commissionRate)}₽
            </div>
          </div>
          <div>
            ...
            <div style={{ fontWeight: 'bold', color: '#2D5016' }}>
              {Math.round(JSON.parse(localStorage.getItem('clientOrders') || '[]')
                .filter(order => order.chefId === chefId)
                .reduce((sum, order) => sum + (order.total || 0), 0) * (1 - progress.commissionRate))}₽
            </div>
          </div>
        </div>
      </div>
                ...
                {Object.entries(tiers).map(([tierKey, tier]) => (
                  <div
                    ...
                    style={{
                      padding: '20px',
                      borderRadius: '15px',
                      border: selectedTier === tierKey ? `3px solid ${tier.color}` : '2px solid #e0e0e0',
                      background: selectedTier === tierKey ? `${tier.color}15` : 'white',
                      cursor: 'pointer',
