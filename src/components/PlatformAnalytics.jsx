...
const PlatformAnalytics = ({ chefId, onAnalyticsUpdate }) => {
  const { showError } = useToast(); // showSuccess не используется
  ...
  const loadAnalytics = useCallback(() => {
    ...
    try {
      ...
      const analyticsData = calculateMetrics(
        filteredOrders,
        chefDishes,
        notifications,
        days
  ...
  }, [chefId, timeRange, timeRanges, showError, onAnalyticsUpdate]);
  ...
  const calculateMetrics = (orders, dishes, notifications, days) => {
    ...
    const uniqueCustomers = new Set(
      orders.map((order) => order.customer?.phone || order.customerId)
    );
    ...
    const repeatOrders = Object.values(repeatCustomers).filter(
      (count) => count > 1
    ).length;
    const retentionRate =
      uniqueCustomers.size > 0
        ? (repeatOrders / uniqueCustomers.size) * 100
        : 0;
    ...
    // SLA метрики
    const slaViolations = notifications.filter(
      (notif) =>
    ...
    const slaCompliance =
      totalOrders > 0
        ? ((totalOrders - slaViolations) / totalOrders) * 100
        : 100;
    ...
    const avgPreparationTime =
      ordersWithTimes > 0
        ? Math.round(totalPreparationTime / ordersWithTimes)
        : 0;
    const avgDeliveryTime =
      ordersWithTimes > 0 ? Math.round(totalDeliveryTime / ordersWithTimes) : 0;
      ...
      >
        ...
        >
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            <div style={{ fontSize: "14px", color: "#666" }}>
              Сколько клиент тратит в среднем
            </div>
          </div>
          <div style={{ fontSize: "12px", color: "#666" }}>
        ...
        >
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            <div style={{ fontSize: "14px", color: "#666" }}>
              Сколько стоит найти нового клиента
            </div>
          </div>
          <div style={{ fontSize: "12px", color: "#666" }}>
        ...
        >
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            <div style={{ fontSize: "14px", color: "#666" }}>
              Сколько посетителей делают заказ
            </div>
          </div>
          <div style={{ fontSize: "12px", color: "#666" }}>
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "center" }}>
            ...
            >
              {formatCurrency(
                (analytics.commissionEarned || 0) -
                  (analytics.marketingSpend || 0)
              )}
            </div>
            <div style={{ fontSize: "14px", color: "#666" }}>
              Чистая прибыль
