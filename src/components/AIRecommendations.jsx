...
const AIRecommendations = ({ dishes, onDishSelect }) => {
  ...
  const analyzeUserPreferences = (orders) => {
    ...
    const cookingTimeSum = orders.reduce(
      (sum, order) => sum + (order.cookingTime || 30),
      0
    );
  ...
  const generateAIReason = (dish, preferences, rank) => {
    const reasons = [
      ...
      `Время приготовления ${
        dish.cookingTime || 30
      } мин соответствует вашим предпочтениям`,
  ...
  const generateAIRecommendations = useCallback(
    (dishes, preferences, userPrefs) => {
      console.log("🤖 Генерация AI рекомендаций:", {
      ...
      const isNewUser =
        ...
        (preferences.favoriteCuisines.includes("tatar") &&
          preferences.favoriteCuisines.includes("russian"));
      ...
      let filteredDishes = dishes.filter((dish) => {
        ...
        if (userPrefs.cuisine !== "all") {
          ...
          if (!preferences.favoriteCuisines.includes(dish.category))
            return false;
        }
        ...
        if (userPrefs.cookingTime !== "all") {
          ...
          switch (userPrefs.cookingTime) {
            ...
            case "slow":
              ...
              break;
            default:
              // Все времена приготовления подходят
              break;
          }
        } else if (!isNewUser) {
          // Для опытных пользователей используем историю заказов
          const timeDiff = Math.abs(
            (dish.cookingTime || 30) - preferences.preferredCookingTime
          );
          if (timeDiff > 30) {
            return false;
          }
  ...
  }, [dishes, generateAIRecommendations, userPreferences]);
      ...
      >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handlePreferenceChange("cookingTime", e.target.value)
            }
            style={{
              width: "100%",
              padding: "4px",
