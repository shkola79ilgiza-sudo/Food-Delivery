...
const ChefPreparations = ({ onClose }) => {
  ...
  const loadPreparations = useCallback(() => {
    try {
      const savedPreparations = JSON.parse(
        localStorage.getItem("chefPreparations") || "[]"
      );
      ...
      if (savedPreparations.length === 0) {
        ...
        localStorage.setItem(
          "chefPreparations",
          JSON.stringify(demoPreparations)
        );
      } else {
  ...
  const updatePreparationStatus = (preparationId, newStatus) => {
    try {
      ...
      localStorage.setItem(
        "chefPreparations",
        JSON.stringify(updatedPreparations)
      );
      ...
      if (preparation) {
        ...
        localStorage.setItem(
          "clientNotifications",
          JSON.stringify(updatedClientNotifications)
        );
  ...
  const getStatusMessage = (status, preparation) => {
    const statusMessages = {
      ...
      in_progress: `Повар начал готовить ваши заготовки. Доставка ${new Date(
        preparation.deliveryDate
      ).toLocaleDateString("ru-RU")} в ${preparation.deliveryTime}`,
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        {filteredPreparations.length === 0 ? (
          ...
          >
            <div
              style={{ fontSize: "64px", marginBottom: "20px", opacity: 0.8 }}
            >
              🥘
            </div>
            <h3
            ...
            {filteredPreparations.map((preparation) => (
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div style={{ color: "#666", fontSize: "14px" }}>
                      📅 Доставка: {formatDate(preparation.deliveryDate)} в{" "}
                      {preparation.deliveryTime}
                    </div>
                  </div>
                  <div
                    style={{
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  >
                    <div>
                      <strong>🥘 Тип:</strong> {preparation.preparationType}
                    </div>
                    <div>
                      <strong>💰 Сумма:</strong> {preparation.totalPrice} ₽
                    </div>
                    <div>
                      <strong>📞 Телефон:</strong> {preparation.contactPhone}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  {preparation.status === "pending" && (
                    <>
                      <button
                        onClick={() =>
                          updatePreparationStatus(preparation.id, "in_progress")
                        }
                        style={{
                          background: "#17a2b8",
                          color: "white",
                      ...
                      <button
                        onClick={() =>
                          updatePreparationStatus(preparation.id, "cancelled")
                        }
                        style={{
                          background: "#dc3545",
                          color: "white",
                  ...
                  {preparation.status === "in_progress" && (
                    <button
                      onClick={() =>
                        updatePreparationStatus(preparation.id, "completed")
                      }
                      style={{
                        background: "#28a745",
                        color: "white",
          ...
          >
            ...
            >
              ...
              >
                ...
                <div>
                  <strong>Тип заготовки:</strong>{" "}
                  {selectedPreparation.preparationType}
                </div>
                <div>
                  <strong>Дата и время доставки:</strong>{" "}
                  {formatDate(selectedPreparation.deliveryDate)} в{" "}
                  {selectedPreparation.deliveryTime}
                </div>
                <div>
