...
const HelpGuestBidding = ({ request, onClose, isChef = false }) => {
    ...
    const demoBids = [
      {
        ...
        chefAvatar: '👩‍🍳',
        price: 4500,
        message: 'Готовлю русскую кухню уже 10 лет. Сделаю ваш праздник незабываемым!',
      ...
      },
      {
        ...
        chefAvatar: '👨‍🍳',
        price: 4200,
        message: 'Специализируюсь на узбекской кухне. Приготовлю настоящий плов и самсу!',
  ...
  const loadBids = () => {
    try {
      const savedBids = JSON.parse(localStorage.getItem(`helpGuestBids_${request.id}`) || '[]');
      setBids(savedBids);
    } catch (error) {
  ...
  const saveBids = (updatedBids) => {
    try {
      localStorage.setItem(`helpGuestBids_${request.id}`, JSON.stringify(updatedBids));
    } catch (error) {
  ...
  const handleAcceptBid = (bidId) => {
    try {
      ...
      localStorage.setItem('chefNotifications', JSON.stringify(chefNotifications));
  ...
  return (
    <div className="help-guest-bidding-overlay" onClick={onClose}>
      <div className="help-guest-bidding-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="help-guest-bidding-content">
          ...
          <div className="request-info">
            ...
            <div className="request-details">
              <p><strong>Бюджет клиента:</strong> {request.budget}₽</p>
              <p><strong>Дата мероприятия:</strong> {new Date(request.eventDate).toLocaleDateString('ru-RU')}</p>
              <p><strong>Количество гостей:</strong> {request.guestCount}</p>
              <p><strong>Адрес:</strong> {request.address}</p>
              {request.specialRequests && (
                <p><strong>Особые пожелания:</strong> {request.specialRequests}</p>
              )}
            </div>
          </div>
            ...
            <div className="bid-form">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setNewBid(prev => ({ ...prev, specialOffers: e.target.value }))}
                  ...
                  <div key={bid.id} className={`bid-card ${bid.status}`}>
                    <div className="bid-header">
                      <div className="chef-info">
                        ...
                        <div>
                          <h5>{bid.chefName}</h5>
                          <p className="bid-time">{formatTime(bid.createdAt)}</p>
                        </div>
                      </div>
                    ...
                    <div className="bid-details">
                      {bid.estimatedTime && (
                        <p><strong>Время:</strong> {bid.estimatedTime}</p>
                      )}
                      {bid.message && (
                        <p><strong>Сообщение:</strong> {bid.message}</p>
                      )}
                      {bid.specialOffers && (
                        <p><strong>Предложения:</strong> {bid.specialOffers}</p>
                      )}
                    </div>
