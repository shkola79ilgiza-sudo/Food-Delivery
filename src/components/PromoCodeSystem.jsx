...
const mockPromoCodes = [
  ...
  },
  {
    ...
    description: "Скидка 25% при заказе от 2000₽",
    discount: 25,
    type: "percentage",
    minOrder: 2000,
    maxDiscount: 1000,
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      if (cartTotal < promo.minOrder) {
        showError(
          `Минимальная сумма заказа для этого промокода: ${promo.minOrder}₽`
        );
        setIsLoading(false);
        return;
      }
      ...
      showSuccess(
        `Промокод "${promo.name}" применён! Скидка: ${Math.round(
          discountAmount
        )}₽`
      );
