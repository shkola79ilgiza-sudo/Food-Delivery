...
const OrderHistoryAnalysis = ({ clientId, role = "client" }) => {
  ...
  const loadOrderHistory = useCallback(() => {
    try {
      ...
      if (allOrders.length === 0) {
        ...
        console.log(
          "📊 OrderHistoryAnalysis: Potential order keys:",
          orderKeys
        );
        ...
        for (const key of orderKeys) {
          try {
            ...
            if (Array.isArray(data) && data.length > 0) {
              console.log(
                `📊 OrderHistoryAnalysis: Found orders in key "${key}":`,
                data
              );
              allOrders = data;
              break;
            ...
            console.log(
              `📊 OrderHistoryAnalysis: Error parsing key "${key}":`,
              e
            );
          }
        }
      ...
      const clientOrders = allOrders.filter(
        (order) =>
          ...
          order.clientId === "demo_client" ||
          !order.clientId
      );

      console.log(
        "📊 OrderHistoryAnalysis: Final loaded orders:",
        clientOrders
      );
      console.log(
        "📊 OrderHistoryAnalysis: Orders count:",
        clientOrders.length
      );
  ...
  const analyzePreferences = (orders) => {
    ...
    // Временные паттерны
    const totalTimeOrders =
      timeCount.morning + timeCount.afternoon + timeCount.evening;
    if (totalTimeOrders > 0) {
      preferences.timePatterns.morning = Math.round(
        (timeCount.morning / totalTimeOrders) * 100
      );
      preferences.timePatterns.afternoon = Math.round(
        (timeCount.afternoon / totalTimeOrders) * 100
      );
      preferences.timePatterns.evening = Math.round(
        (timeCount.evening / totalTimeOrders) * 100
      );
    }

    // Сезонные предпочтения
    const totalSeasonOrders = Object.values(seasonCount).reduce(
      (a, b) => a + b,
      0
    );
    if (totalSeasonOrders > 0) {
      preferences.seasonalPreferences.spring = Math.round(
        (seasonCount.spring / totalSeasonOrders) * 100
      );
      preferences.seasonalPreferences.summer = Math.round(
        (seasonCount.summer / totalSeasonOrders) * 100
      );
      preferences.seasonalPreferences.autumn = Math.round(
        (seasonCount.autumn / totalSeasonOrders) * 100
      );
      preferences.seasonalPreferences.winter = Math.round(
        (seasonCount.winter / totalSeasonOrders) * 100
      );
    }
  ...
  const generatePersonalizedRecommendations = (preferences, orders) => {
    ...
    if (favoriteDishes.length > 0) {
      recommendations.push({
        ...
        description: `Вы часто заказываете: ${favoriteDishes
          .slice(0, 3)
          .join(", ")}`,
    ...
    const mostActiveTime = Object.entries(preferences.timePatterns).sort(
      ([, a], [, b]) => b - a
    )[0];

    if (mostActiveTime && mostActiveTime[1] > 50) {
      const timeName =
        mostActiveTime[0] === "morning"
          ? "утром"
          : mostActiveTime[0] === "afternoon"
          ? "днем"
          : "вечером";
      recommendations.push({
        type: "time_based",
    ...
    if (seasonPercentage > 30) {
      ...
      recommendations.push({
        ...
        description: `В ${getSeasonName(
          currentSeason
        ).toLowerCase()} вы особенно активны в заказах`,
  ...
  const performAnalysis = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.log(
          "📊 OrderHistoryAnalysis: Preferences analyzed:",
          clientPreferences
        );
        ...
        console.log(
          "📊 OrderHistoryAnalysis: Recommendations generated:",
        ...
        showSuccess(
          `Анализ завершен! Найдено ${personalizedRecommendations.length} рекомендаций`
        );
      } catch (error) {
  ...
  return (
    <div
      className="order-history-analysis"
      style={{
      ...
      {orderHistory.length === 0 ? (
        ...
        >
          ...
          <div style={{ fontSize: "14px" }}>
            Сделайте первый заказ, чтобы получить персонализированные
            рекомендации
          </div>
        </div>
      ) : (
        <div className="analysis-content">
          ...
          >
            ...
            >
              <div style={{ fontSize: "24px", fontWeight: "bold" }}>
                {preferences
                  ? Object.keys(preferences.favoriteDishes).length
                  : "—"}
              </div>
