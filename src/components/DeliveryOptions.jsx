...
const DeliveryOptions = ({ order, onDeliveryOptionChange }) => {
  ...
  const [selectedOption, setSelectedOption] = useState(order?.deliveryOption || 'pickup');
  ...
  }, [selectedOption, deliveryAddress, deliveryRadius]);
  ...
  const calculateDeliveryDetails = () => {
    ...
    if (selectedOption === 'chef_delivery') {
      ...
      estimatedTime = `${30 + deliveryRadius * 2}-${60 + deliveryRadius * 2} мин`;
    }

    // Дополнительные расчеты для курьерской службы
    if (selectedOption === 'courier_partner') {
      // Имитация расчета на основе адреса
      if (deliveryAddress.includes('центр') || deliveryAddress.includes('центр')) {
  ...
  const simulateCourierAssignment = () => {
    ...
    showSuccess(`Курьер ${assignedCourier.name} назначен! ETA: ${assignedCourier.eta}`);
  };
  ...
  return (
    <div style={{
      ...
      <h3 style={{ color: '#2D5016', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        🚚 Опции доставки
      </h3>
          ...
          {deliveryOptions.map(option => (
            <div
              ...
              style={{
                padding: '15px',
                borderRadius: '12px',
                border: selectedOption === option.id ? `3px solid ${option.color}` : '2px solid #e0e0e0',
                background: selectedOption === option.id ? `${option.color}15` : 'white',
                cursor: 'pointer',
          ...
          {courierInfo && (
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#856404' }}>
                <div><strong>Имя:</strong> {courierInfo.name}</div>
                <div><strong>Рейтинг:</strong> ⭐ {courierInfo.rating}</div>
                <div><strong>Транспорт:</strong> {courierInfo.vehicle}</div>
                <div><strong>ETA:</strong> {courierInfo.eta}</div>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        <h4 style={{ color: selectedDeliveryOption.color, marginBottom: '10px' }}>
          📋 Итоговая информация
        </h4>
        <div style={{
          display: 'grid',
          ...
          <div>
            <strong>Стоимость:</strong> {deliveryFee === 0 ? 'Бесплатно' : `${deliveryFee}₽`}
          </div>
          <div>
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>📋 SLA гарантии:</div>
