...
const ChefRatings = ({ onClose }) => {
  ...
  const loadChefStats = useCallback(async () => {
    try {
      setLoading(true);
  ...
  }, [showError, calculateChefStats]);

  const calculateChefStats = useCallback((orders, reviews) => {
    const now = new Date();
    ...
    const monthlyRevenue = ordersLastMonth.reduce(
      (sum, order) => sum + (order.total || 0),
      0
    );
    ...
    const stability =
      orders.length > 0 ? (successfulOrders.length / orders.length) * 100 : 0;
          ...
          <div className="chef-stats-section">
            ...
            <div className="stats-grid">
              <div className="stat-card">
                ...
                <div className="stat-info">
                  ...
                  <div className="stat-detail">
                    {chefStats.totalReviews} отзывов
                  </div>
                </div>
              </div>
              ...
              <div className="stat-card">
                ...
                <div className="stat-info">
                  <div className="stat-number">
                    {chefStats.monthlyRevenue.toLocaleString()}₽
                  </div>
                  <div className="stat-label">Выручка за месяц</div>
                  <div className="stat-detail">
                    {chefStats.ordersLastMonth} заказов
                  </div>
                </div>
              </div>
              ...
              <div className="stat-card">
                ...
                <div className="stat-info">
                  <div className="stat-number">
                    {chefStats.avgCompletionTime} мин
                  </div>
                  <div className="stat-label">Среднее время готовки</div>
                  <div className="stat-detail">
                    {chefStats.stability.toFixed(1)}% успешных
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="motivation-messages">
              {chefStats.averageRating >= 4.5 && (
                <div className="motivation-card success">
                  ...
                  <div className="motivation-text">
                    <strong>Отлично!</strong> Ваш рейтинг{" "}
                    {chefStats.averageRating.toFixed(1)} - это результат
                    качественной работы!
                  </div>
                </div>
              )}
              ...
              {chefStats.ordersLastMonth >= 30 && (
                <div className="motivation-card success">
                  <div className="motivation-icon">🚀</div>
                  <div className="motivation-text">
                    <strong>Активность!</strong> {chefStats.ordersLastMonth}{" "}
                    заказов за месяц - вы на правильном пути!
                  </div>
                </div>
              )}
              ...
              {chefStats.stability >= 90 && (
                <div className="motivation-card success">
                  ...
                  <div className="motivation-text">
                    ...
                    {chefStats.stability.toFixed(1)}% успешных заказов - клиенты
                    доверяют вам!
                  </div>
                </div>
              )}
              ...
              {chefStats.averageRating < 4.0 && (
                <div className="motivation-card warning">
                  <div className="motivation-icon">📈</div>
                  <div className="motivation-text">
                    <strong>Развивайтесь!</strong> Ваш рейтинг{" "}
                    {chefStats.averageRating.toFixed(1)} - есть куда расти!
                  </div>
                </div>
              )}
            </div>
