...
const VIPSystem = () => {
  ...
  const vipLevels = [
    ...
    },
    {
      ...
      minOrders: 15,
      minSpent: 15000,
      benefits: ['Персональный менеджер', 'Экспресс доставка', 'Скидка 10%', 'Эксклюзивные блюда'],
      discount: 10
    },
    {
      name: 'Platinum',
      ...
      minOrders: 30,
      minSpent: 30000,
      benefits: ['VIP поддержка 24/7', 'Бесплатная доставка', 'Скидка 15%', 'Персональные предложения'],
      discount: 15
    },
    {
      name: 'Diamond',
      ...
      minOrders: 50,
      minSpent: 50000,
      benefits: ['Консьерж-сервис', 'Бесплатная доставка', 'Скидка 20%', 'Эксклюзивные мероприятия'],
      discount: 20
    }
  ];
  ...
  const getNextLevel = () => {
    const currentLevelIndex = vipLevels.findIndex(level => 
      userStats.totalOrders >= level.minOrders && 
      userStats.totalSpent >= level.minSpent
    );
    
    return currentLevelIndex < vipLevels.length - 1 ? vipLevels[currentLevelIndex + 1] : null;
  };
  ...
  const calculateProgress = () => {
    ...
    const ordersProgress = Math.min(
      ((userStats.totalOrders - currentLevel.minOrders) / 
       (nextLevel.minOrders - currentLevel.minOrders)) * 100, 100
    );
    
    const spentProgress = Math.min(
      ((userStats.totalSpent - currentLevel.minSpent) / 
       (nextLevel.minSpent - currentLevel.minSpent)) * 100, 100
    );
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '15px' }}>
            <div style={{
              ...
              <span>Заказы: {userStats.totalOrders} / {nextLevel.minOrders}</span>
          ...
          <div>
            <div style={{
              ...
              <span>Потрачено: {userStats.totalSpent.toLocaleString()}₽ / {nextLevel.minSpent.toLocaleString()}₽</span>
          ...
          {achievements.map((achievement) => (
            <div
              ...
              style={{
                ...
                border: `2px solid ${achievement.unlocked ? '#c3e6cb' : '#e0e0e0'}`,
                borderRadius: '15px',
                padding: '15px',
                textAlign: 'center',
            ...
            >
              ...
              <div style={{
                ...
                {achievement.unlocked ? '✅ Разблокировано' : `+${achievement.points} очков`}
              </div>
            </div>
          ))}
        </div>
