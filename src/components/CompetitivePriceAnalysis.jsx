...
const CompetitivePriceAnalysis = ({ dishData, onPriceRecommendation }) => {
  ...
  const runAnalysis = async () => {
    ...
    try {
      const result = await competitivePriceAnalyzer.analyzeCompetitivePricing(dishData);
      setAnalysis(result);
      
      // Передаем рекомендацию по цене родительскому компоненту
      if (onPriceRecommendation && result.recommendations.length > 0) {
        const priceRecommendation = result.recommendations.find(r => r.suggestedPrice);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: '5px 0 0 0', color: '#6c757d', fontSize: '14px' }}>
            {analysis.dishName} • Уверенность: {Math.round(analysis.confidence * 100)}%
          </p>
        </div>
      </div>
      ...
      {/* Общий балл */}
      <div style={{
        background: analysis.overallScore >= 70 ? '#d4edda' : analysis.overallScore >= 50 ? '#fff3cd' : '#f8d7da',
        padding: '15px',
        borderRadius: '8px',
        marginBottom: '20px',
        textAlign: 'center'
        ...
        <div style={{ fontSize: '14px', color: '#6c757d', marginTop: '5px' }}>
          {analysis.overallScore >= 70 ? 'Отличная позиция' : 
           analysis.overallScore >= 50 ? 'Хорошая позиция' : 'Требует внимания'}
        </div>
      </div>
      ...
      {analysis.competitors.restaurants.count > 0 && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{
            ...
            <div style={{
              background: analysis.priceAnalysis.vsRestaurants.advantage.level === 'high' ? '#d4edda' : 
                         analysis.priceAnalysis.vsRestaurants.advantage.level === 'medium' ? '#fff3cd' : '#f8d7da',
              padding: '15px',
              borderRadius: '8px',
              textAlign: 'center'
            }}>
              <div style={{ fontSize: '12px', color: '#6c757d', marginBottom: '5px' }}>
                {analysis.priceAnalysis.vsRestaurants.advantage.absolute > 0 ? 'Экономия' : 'Переплата'}
              </div>
              <div style={{ 
                fontSize: '20px', 
                fontWeight: 'bold',
                color: analysis.priceAnalysis.vsRestaurants.advantage.absolute > 0 ? '#28a745' : '#dc3545'
              }}>
                {Math.abs(analysis.priceAnalysis.vsRestaurants.advantage.absolute)}₽
              </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span>Ресторанов проанализировано: {analysis.competitors.restaurants.count}</span>
          <span>Магазинов проанализировано: {analysis.competitors.stores.count}</span>
        </div>
