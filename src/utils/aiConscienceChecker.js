...
class AIConscienceChecker {
  constructor() {
    this.dietaryStandards = {
      vegetarian: {
        forbidden: ['мясо', 'курица', 'говядина', 'свинина', 'баранина', 'рыба', 'морепродукты', 'креветки', 'краб', 'лосось', 'тунец', 'колбаса', 'сосиски', 'бекон', 'ветчина', 'сало'],
        allowed: ['овощи', 'фрукты', 'зерновые', 'бобовые', 'орехи', 'семена', 'молочные продукты', 'яйца'],
        strictness: 'medium'
      },
      vegan: {
        forbidden: ['мясо', 'курица', 'говядина', 'свинина', 'баранина', 'рыба', 'морепродукты', 'молоко', 'сыр', 'творог', 'йогурт', 'сметана', 'масло', 'сливки', 'яйца', 'майонез', 'мед'],
        allowed: ['овощи', 'фрукты', 'зерновые', 'бобовые', 'орехи', 'семена', 'тофу', 'соевое молоко'],
        strictness: 'high'
      },
      halal: {
        forbidden: ['свинина', 'алкоголь', 'желатин свиной', 'сало', 'бекон', 'ветчина', 'колбаса свиная'],
        allowed: ['говядина', 'баранина', 'курица', 'рыба', 'овощи', 'фрукты', 'зерновые'],
        strictness: 'high'
      },
      kosher: {
        forbidden: ['свинина', 'моллюски', 'ракообразные', 'смешение мяса и молока', 'алкоголь некошерный'],
        allowed: ['говядина кошерная', 'курица кошерная', 'рыба с чешуей', 'овощи', 'фрукты'],
        strictness: 'high'
      },
      gluten_free: {
        forbidden: ['пшеница', 'рожь', 'ячмень', 'овёс', 'глютен', 'мука пшеничная', 'хлеб', 'макароны', 'паста'],
        allowed: ['рис', 'кукуруза', 'картофель', 'овощи', 'фрукты', 'мясо', 'рыба'],
        strictness: 'high'
      }
    };
    ...
    this.ethicalStandards = {
      organic: {
        forbidden: ['пестициды', 'гербициды', 'гмо', 'генетически модифицированный', 'химические удобрения'],
        indicators: ['органический', 'био', 'эко', 'без пестицидов'],
        strictness: 'medium'
      },
      fair_trade: {
        indicators: ['справедливая торговля', 'fair trade', 'поддержка фермеров'],
        strictness: 'low'
      },
      sustainable: {
        forbidden: ['пальмовое масло', 'вырубка лесов', 'чрезмерный вылов'],
        indicators: ['устойчивое производство', 'экологически чистый', 'местные продукты'],
        strictness: 'medium'
      },
      cruelty_free: {
        forbidden: ['тестирование на животных', 'животные продукты из промышленного производства'],
    ...
    this.nutritionalPurity = {
      artificial_additives: ['консерванты', 'красители', 'усилители вкуса', 'глутамат натрия', 'e621', 'e250', 'e252'],
      natural_indicators: ['натуральный', 'без консервантов', 'без красителей', 'домашний', 'свежий'],
      processing_levels: {
  ...
  async analyzeDish(dish, claimedStandards = []) {
    console.log('🔍 Starting AI Conscience Analysis...', { dish: dish.name, claimedStandards });
    ...
    try {
      ...
      analysis.results.dietaryCompliance = await this.checkDietaryCompliance(dish, claimedStandards);
      
      // 2. Проверяем этические стандарты
      analysis.results.ethicalCompliance = await this.checkEthicalCompliance(dish);
      
      // 3. Анализируем натуральность
      analysis.results.nutritionalPurity = await this.analyzeNutritionalPurity(dish);
      ...
      analysis.results.overallScore = this.calculateOverallScore(analysis.results);
      
      // 6. Генерируем рекомендации
      analysis.results.recommendations = await this.generateRecommendations(analysis.results);
      ...
      standardRules.forbidden.forEach(forbiddenIngredient => {
        if (this.containsIngredient(ingredients, forbiddenIngredient)) {
          violations.push({
            ...
            severity: standardRules.strictness === 'high' ? 'critical' : 'warning',
    ...
    Object.entries(this.ethicalStandards).forEach(([standard, rules]) => {
      ...
      if (rules.forbidden) {
        rules.forbidden.forEach(forbiddenElement => {
          if (this.containsIngredient(ingredients, forbiddenElement) || 
      ...
      compliance[standard] = {
        ...
        score: this.calculateEthicalScore(violations, indicators, rules.strictness),
  ...
  async analyzeNutritionalPurity(dish) {
    ...
    this.nutritionalPurity.artificial_additives.forEach(additive => {
      if (this.containsIngredient(ingredients, additive) || 
    ...
    analysis.processingLevel = this.determineProcessingLevel(description, ingredients);
    ...
    if (analysis.processingLevel === 'high') {
      analysis.recommendations.push({
        type: 'info',
        message: 'Блюдо подвергнуто интенсивной обработке. Рассмотрите более натуральные альтернативы.'
      });
    }
  ...
  async checkHealthWarnings(dish) {
    ...
    if (dish.dishSodium && dish.dishSodium > this.healthWarnings.high_sodium.threshold) {
      warnings.push({
        type: 'sodium_warning',
        level: 'high',
    ...
    if (dish.dishSugar && dish.dishSugar > this.healthWarnings.high_sugar.threshold) {
      warnings.push({
        type: 'sugar_warning',
        level: 'high',
    ...
    if (dish.dishFiber && dish.dishFiber < this.healthWarnings.low_fiber.threshold) {
      warnings.push({
        type: 'fiber_warning',
        level: 'low',
  ...
  async generateRecommendations(results) {
    ...
    Object.entries(results.dietaryCompliance).forEach(([standard, compliance]) => {
      if (!compliance.compliant) {
        recommendations.push({
          type: 'dietary',
          priority: 'high',
          standard: standard,
          message: `Уберите запрещенные для ${standard} ингредиенты: ${compliance.violations.map(v => v.ingredient).join(', ')}`,
    ...
    Object.entries(results.ethicalCompliance).forEach(([standard, compliance]) => {
      if (compliance.score < 70) {
        recommendations.push({
          type: 'ethical',
          priority: 'medium',
  ...
  extractIngredients(dish) {
    ...
    if (dish.ingredients) {
      ingredients.push(...dish.ingredients.split(',').map(ing => ing.trim().toLowerCase()));
    }
  ...
  containsIngredient(ingredients, ingredient) {
    ...
    return ingredients.some(ing => ing.includes(searchTerm) || searchTerm.includes(ing));
  }
  ...
  async analyzeMultipleDishes(dishes, claimedStandards = []) {
    console.log(`🔍 Analyzing ${dishes.length} dishes for conscience compliance...`);
    ...
    claimedStandards.forEach(standard => {
      const compliantDishes = results.dishes.filter(d => 
        d.results && d.results.dietaryCompliance[standard] && 
    ...
    results.dishes.forEach(dish => {
      if (dish.results && dish.results.overallScore < 50) {
        results.summary.criticalIssues.push({
          ...
          issues: dish.results.recommendations.filter(r => r.priority === 'high')
        });
      }
    });
