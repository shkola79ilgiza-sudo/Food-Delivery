...
// import { calorizatorInspiredDatabase, findProductInDatabase, searchProducts } from './calorizatorInspiredDatabase.js'; // Не используется в текущей реализации
...
export class SmartNutritionCalculator {
  ...
  parseIngredients(text) {
    ...
    const rawIngredients = normalized
      .split(/[,;.\n]/)
      .map((ing) => ing.trim())
  ...
  extractQuantity(ingredient) {
    ...
    const rangeMatch = ingredient.match(
      /(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)/
    );
    if (rangeMatch) {
    ...
    const decimalMatch = ingredient.match(
      /(\d+\.\d+)\s*(г|кг|мл|л|шт|штук|кусочков?)?/
    );
    if (decimalMatch) {
    ...
    // Ищем целые числа
    const numberMatch = ingredient.match(
      /(\d+)\s*(г|кг|мл|л|шт|штук|кусочков?|столовая ложка|ч\.л\.|стакан|чашка|пучок|веточка|лист|зубчик|щепотка)?/
    );
    if (numberMatch) {
  ...
  extractCookingMethod(ingredient) {
    ...
    for (const method of Object.keys(cookingMethods)) {
      // Убираем неиспользуемую переменную _
  ...
  cleanIngredientName(ingredient) {
    return ingredient
      .replace(
        /\d+(?:\.\d+)?\s*(г|кг|мл|л|шт|штук|кусочков?|столовая ложка|ч\.л\.|стакан|чашка|пучок|веточка|лист|зубчик|щепотка)/g,
        ""
      )
      .replace(
        /\b(вареный|жареный|запеченный|тушеный|сырой|свежий|сушеный|маринованный|соленый|сладкий)\b/g,
        ""
      )
      .replace(
        /\b(мелко|крупно|тонко|толсто)\s*(нарезанный|нарезанный|натертый|натертый)\b/g,
        ""
      )
      .replace(
        /\b(по\s+вкусу|немного|чуть-чуть|совсем\s+немного|щепотка|горсть|несколько|пара|парочка)\b/g,
        ""
      )
      .replace(/\s+/g, " ")
      .trim();
  ...
  findIngredientNutrition(ingredientName) {
    ...
    const mlResult = ingredientML.smartRecognize(
      ingredientName,
      nutritionDatabase
    );
    ...
    const attemptedKeys = Object.keys(nutritionDatabase).filter(
      (key) =>
  ...
  validateResult(result) {
    ...
    if (result.calories > 5000) {
      warnings.push(
        "Очень высокое содержание калорий. Проверьте количество ингредиентов."
      );
    }

    if (result.protein > 500) {
      warnings.push(
        "Очень высокое содержание белка. Проверьте количество ингредиентов."
      );
    }

    if (result.carbs > 1000) {
      warnings.push(
        "Очень высокое содержание углеводов. Проверьте количество ингредиентов."
      );
    }

    if (result.fat > 500) {
      warnings.push(
        "Очень высокое содержание жиров. Проверьте количество ингредиентов."
      );
    }
    ...
    if (
      ...
      warnings.push(
        "Не удалось рассчитать питательную ценность. Проверьте правильность написания ингредиентов."
      );
    }
  ...
  getRecommendations(result) {
    ...
    if (result.warnings.length > 0) {
      recommendations.push(
        "Проверьте предупреждения и скорректируйте значения"
      );
    }
