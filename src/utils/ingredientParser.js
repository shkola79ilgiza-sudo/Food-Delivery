...
export class IngredientParser {
  ...
  initializeQuantityPatterns() {
    return [
      ...
      {
        pattern: /^(\d+)\s*(кг|килограмм|kg|kilogram)$/i,
        type: "weight",
        multiplier: 1000,
      },
      {
        pattern: /^(\d+)\s*(мл|миллилитр|ml|milliliter)$/i,
        type: "volume",
        multiplier: 1,
      },
      {
        pattern: /^(\d+)\s*(л|литр|l|liter)$/i,
        type: "volume",
        multiplier: 1000,
      },
      {
        pattern: /^(\d+)\s*(шт|штук|piece|pcs)$/i,
        type: "count",
        multiplier: 1,
      },
      {
        pattern: /^(\d+)\s*(ст\.л|столовая ложка|tbsp)$/i,
        type: "volume",
        multiplier: 15,
      },
      {
        pattern: /^(\d+)\s*(ч\.л|чайная ложка|tsp)$/i,
        type: "volume",
        multiplier: 5,
      },
      ...
      {
        pattern: /^(\d+)-(\d+)\s*(г|грамм|g)$/i,
        type: "range_weight",
        multiplier: 1,
      },
      {
        pattern: /^(\d+)-(\d+)\s*(шт|штук|piece)$/i,
        type: "range_count",
        multiplier: 1,
      },
      ...
      {
        pattern: /^(немного|чуть-чуть|щепотка|pinch)$/i,
        type: "small_amount",
        multiplier: 1,
      },
      {
        pattern: /^(много|множество|handful)$/i,
        type: "large_amount",
        multiplier: 100,
      },
  ...
  parseIngredients(text) {
    ...
    const accuracy =
      totalProcessed > 0 ? (successfullyParsed / totalProcessed) * 100 : 0;
    ...
    for (const pattern of this.quantityPatterns) {
      ...
      if (match) {
        switch (pattern.type) {
          ...
          default:
            // Обработка неизвестного типа паттерна
            return null;
        }
      }
    }
  ...
  extractIngredientName(line, quantityMatch) {
    ...
    name = name.replace(
      /(г|грамм|кг|килограмм|мл|миллилитр|л|литр|шт|штук|ст\.л|столовая ложка|ч\.л|чайная ложка|стакан)/gi,
      ""
    );
    name = name.replace(/-/g, "");
  ...
  validateQuantity(quantity) {
    ...
    if (quantity.type === "range") {
      ...
      if (quantity.min > quantity.max) {
        return {
          valid: false,
          message: "Минимальное значение больше максимального",
        };
      }
    }
